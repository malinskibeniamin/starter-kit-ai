...
function Table({ className, variant, size, ...props }: TableProps) {
  return (
    <div data-slot="table-container" className={cn(tableVariants({ variant, size }))}>
...
function TableHead({ className, align, width, ...props }: TableHeadProps) {
  return <th data-slot="table-head" className={cn(tableHeadVariants({ align, width }), className)} {...props} />;
}

interface TableCellProps extends Omit<React.ComponentProps<'td'>, 'align'>, VariantProps<typeof tableCellVariants> {}

function TableCell({ className, align, weight, truncate, ...props }: TableCellProps) {
  return (
    <td data-slot="table-cell" className={cn(tableCellVariants({ align, weight, truncate }), className)} {...props} />
  );
}
