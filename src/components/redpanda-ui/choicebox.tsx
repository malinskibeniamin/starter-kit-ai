...
import { RadioGroup } from '@/components/redpanda-ui/radio-group';
...
export type ChoiceboxItemProps = RadioGroupPrimitive.RadioGroupItemProps & { testId?: string };

export const ChoiceboxItem = ({ className, children, testId, ...props }: ChoiceboxItemProps) => (
  <RadioGroupPrimitive.Item
    className={cn(
      'text-left',
      '[&[data-state="checked"]]:border-selected',
      '[&[data-state="checked"]]:bg-selected/5',
      'hover:border-selected/50',
    )}
    asChild
    {...props}
  >
    <Card
      className={cn(
        'flex cursor-pointer flex-row items-start justify-between rounded-md p-4 shadow-none transition-all border-2',
        className,
      )}
      data-testid={testId}
    >
