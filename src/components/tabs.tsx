...
type TabsProps = React.ComponentProps<typeof TabsPrimitive.Root> & VariantProps<typeof tabsVariants>;

function Tabs({ className, size, variant, ...props }: TabsProps) {
  return <TabsPrimitive.Root data-slot="tabs" className={cn(tabsVariants({ size, variant }), className)} {...props} />;
}
...
const tabsListVariants = cva('text-muted-foreground inline-flex h-10 items-center justify-center', {
  variants: {
    variant: {
      ...
      underline: 'w-full relative justify-start rounded-t-xl bg-muted border-b border-border text-current py-0 px-4',
    },
    layout: {
      auto: '',
      equal: 'grid',
...
function TabsTrigger({ className, value, variant, ...props }: TabsTriggerProps) {
  return (
